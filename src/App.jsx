  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section className="section contact-section wrap" id="contact"><div className="section-label"><span>05</span><span>CONTACT</span></div><div className="contact-grid"><div><h2>Contact Me</h2><p className="large-copy">Have an opportunity, a question, or just want to talk data? My inbox is always open.</p><div className="contact-details"><a href="mailto:kuruvasivasiva1417@gmail.com"><Mail size={17} /> kuruvasivasiva1417@gmail.com</a><a href="tel:+918555820953"><Mail size={17} /> 8555820953</a><span><MapPin size={17} /> Andhra Pradesh, India</span></div></div><form className="contact-form" onSubmit={(event) => { event.preventDefault(); setSent(true) }}><label>Name<input type="text" placeholder="Your name" required /></label><label>Email<input type="email" placeholder="you@example.com" required /></label><label>Message<textarea placeholder="Tell me a little about it..." rows="4" required /></label><button className="button primary" type="submit">{sent ? 'Message sent' : 'Send message'} {sent ? <Check size={17} /> : <Send size={16} />}</button></form></div></section>
      </main>
      <footer className="footer wrap"><a className="brand" href="#home"><span>KS</span><strong>Kuruva Siva</strong></a><p>Designed &amp; built with curiosity. © 2025</p><div className="socials"><a href="#contact" aria-label="GitHub"><Link size={18} /></a><a href="#contact" aria-label="Instagram"><CircleUserRound size={18} /></a></div></footer>
    </div>
  )
}
