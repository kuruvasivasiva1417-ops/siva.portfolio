  ...
  return (
    <div className="site-shell">
      ...
      <main>
        ...
        <section className="section projects-section wrap" id="projects"><div className="section-label"><span>03</span><span>SELECTED WORK</span></div><div className="section-heading"><h2>Projects with a <em>purpose.</em></h2><p>Academic experiments and personal builds that keep me learning.</p></div><div className="project-grid">{projects.map(({ number, title, tag, description, icon: Icon, color }) => <article className={`project-card ${color}`} key={title}><div className="project-art"><span className="project-number">{number}</span><Icon size={48} strokeWidth={1} /><span className="art-grid" /></div><div className="project-body"><span className="project-tag">{tag}</span><h3>{title}</h3><p>{description}</p><div className="project-links"><a href="#contact" aria-label={`Ask about ${title}`}>Discuss project <ArrowUpRight size={15} /></a></div></div></article>)}</div></section>
        ...
        <section className="section contact-section wrap" id="contact"><div className="section-label"><span>05</span><span>CONTACT</span></div><div className="contact-grid"><div><h2>Contact Me</h2><p className="large-copy">Have an opportunity, a question, or just want to talk data? My inbox is always open.</p><div className="contact-details"><a href="mailto:kuruvasivasiva1417@gmail.com"><Mail size={17} /> kuruvasivasiva1417@gmail.com</a><a href="tel:+918555820953"><Phone size={17} /> 8555820953</a><span><MapPin size={17} /> Andhra Pradesh, India</span></div></div><form className="contact-form" onSubmit={(event) => { event.preventDefault(); setSent(true) }}><label>Name<input type="text" placeholder="Your name" required /></label><label>Email<input type="email" placeholder="you@example.com" required /></label><label>Message<textarea placeholder="Tell me a little about it..." rows="4" required /></label><button className="button primary" type="submit" aria-live="polite">{sent ? 'Message sent' : 'Send message'} {sent ? <Check size={17} /> : <Send size={16} />}</button></form></div></section>
      </main>
      <footer className="footer wrap"><a className="brand" href="#home"><span>KS</span><strong>Kuruva Siva</strong></a><p>Designed &amp; built with curiosity. © {new Date().getFullYear()}</p><div className="socials"><a href="#contact" aria-label="GitHub"><Link size={18} /></a><a href="#contact" aria-label="Instagram"><CircleUserRound size={18} /></a></div></footer>
    </div>
  )
}
